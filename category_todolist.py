...
class ProjectTasksDialog(QDialog):
    ...
    def __init__(self, project_id):
        ...
        self.setWindowTitle("Category Tasks")
        
        self.project_label = QLabel("<b>Category:</b>")
        self.tasks_label = QLabel("<b>Tasks:</b>")

        self.project_labels_list = ["CategoryID", "Decription", "Duedate", "Created", "Completed"]
    ...
    def get_project_details(self,user):
        project_details = []
        controller_results = self.controller.get_single_project(self.project_id,user)
        for item in controller_results[0][2:3]:
            if item == None:
                item = ""
    ...
    def populate_project_tasks_table(self):
        table_items = self.project_tasks_table.get_project_tasks(user)
